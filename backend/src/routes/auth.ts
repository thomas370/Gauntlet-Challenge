...
router.get("/steam/callback", async (req, res) => {
  ...
  // Reconstruct a URLSearchParams from req.query (Express parses ?a=b into req.query).
  ...
  try {
    ...
    res.cookie(attrs.name, token, {
      ...
      path: attrs.path,
      maxAge: attrs.maxAge * 1000, // express attend des ms
    });
    console.log("[steam/callback] cookie set, redirecting to /lobby");
    // Redirection relative — même origine (frontend statique servi par Express).
    res.redirect("/lobby");
  } catch (err) {
